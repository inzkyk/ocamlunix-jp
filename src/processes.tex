...
プロセスは \indexlibvalue{Unix}{getpid} 関数を呼ぶことでプロセス ID を取得できます。
...
同様にファイルディスクリプタも \ml+fork+ が呼ばれたときにコピーされます。
...
% it is time to leave.
\ml+leave hhmm+ コマンドは時刻 \ml+hhmm+ に利用を終える時間だと報告するバックグラウンドプロセスを
...
\indexlibvalue{Unix}{time} 関数はエポック (1970 年 1 月 1 日 午前 0 時 0 分 0 秒) から現在時刻までの
経過秒数を返します。 \indexlibvalue{Unix}{localtime} を使うとこの値から年、月、日、時、分、秒を計算することができます。
...
基礎となるシステムコールは \syscall{waitpid} であり、 \ml+wait ()+ という呼び出しは
...
返り値の最初の要素は \ml+wait+ によって終了を捕捉された子プロセスのプロセス ID です。
2番目の要素は以下に示す \libtype{Unix}{process\_status} 型の値です:
%
\begin{mltypecases}
...
そうでなければ親プロセスは子プロセスのどれかが終了するまでブロックします (\quotes{ランデブー} と呼ばれる動作です)。
...
\ml+waitpid+ 関数は二つのオプショナルなフラグを第一引数に受け取ります。
...
\ml+wait+ はプロセス間の同期を行いますが、それ以外に子プロセスが持つリソースの完全な開放も行います。
終了した子プロセスは \quotes{ゾンビ} 状態となり大部分のリソース (メモリなど) が開放されますが、
...
この仕組みによって便利な \quotes{ダブルフォーク} テクニックが使えるようになります。
...
% システムコール \syscall{execve}、 \syscall{execv}、 そして \syscall{execvp} は
現在のプロセスでプログラムを起動します。
...
\ml+input_line+ 関数は EOF に達すると \ml+End_of_file+ を送出するので、これをもってループの終了とします。
