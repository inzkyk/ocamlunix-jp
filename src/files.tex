...
\ml+Filename+ モジュールには \libvalue{Filename}{current\_dir\_name} と
...
それぞれ \syscall{getlogin} と \syscall{getgroups} 関数で取得できます。
%
\begin{listingcodefile}{tmpunix.mli}
...
\ml+s+ ビットが立っている実行可能ファイルを実行すると、ファイルの所有者または所有グループとして
ファイルが実行されます。
...
\syscall{closedir} はディレクトリディスクリプタを閉じます。
\begin{example}
...
\ml+handler+ 関数は探索中に起こった \ml+Unix_error+ 型のエラーを報告します。
...
一秒間にコピーできるバイト数を縦軸に、ブロックサイズ (\ml+buffer_size+ の値) を横軸に示しています。
\begin{myfigure}
\begin{myimage}[width="100\%"]
...
% standard.
端末と擬似端末はキャラクタタイプのスペシャルファイルで、 \ocaml から設定を変更することができます。
...
システムコール \syscall{lockf} を使うとファイルの一部分をロックするより良い同期パターンを利用できます。
%
\begin{codefile}{tmpunix.mli}
...
通常ユーザが \ml+chown+ を実行すると \quotes{permission denied} エラーが出て失敗することがありますが、
このエラーは捕捉した上で無視します。
...
そこで \ml+really_read+ を再利用します。アーカイブが壊れていない限り、
