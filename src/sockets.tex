...
クライアント-サーバモデルを図~\ref{fig/client-server} に示します。
...
ソケットの \emph{通信方式} は通信が信頼できるか (データの消失や重複があるか)、
...
例えば \ml+ping+ コマンドは \textsc{icmp} (\emph{Internet Control Message Protocol}) を使って、
...
サーバにデータを送信し、\indexvalue{read} はサーバからのデータを受信します。
...
この意味で、 \ml+client+ コマンドは \quotes{ユニバーサルな} クライアントアプリケーションです。
...
関数を直接呼び出すことは当然できません。
ですが、マシン B に接続して関数を実行し、結果をマシン A に送り返すという
...
プロトコルの効率が重要でないネットワークサービスでは \quotes{テキスト} プロトコルが
よく利用されます。
...
\http/1.1 で使われる複雑なリクエストではサーバは全ての応答の先頭に
...
\item \textit{\ml{body}} が一つのブロックの場合、
